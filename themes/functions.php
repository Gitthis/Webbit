...
function get_debug() {
  ...
  if(isset($ly->config['debug']['webbit']) && $ly->config['debug']['webbit']) {
  ...
  if(isset($ly->config['debug']['session']) && $ly->config['debug']['session']) {
    $html .= "<hr><h3>SESSION</h3><p>The content of CWebbit->session:</p><pre>" . htmlent(print_r($ly->session, true)) . "</pre>";
...
function get_messages_from_session() {
  $messages = CWebbit::Instance()->session->GetMessages();
  $html = null;
  if(!empty($messages)) {
...
function get_gravatar($size=null) {
  return 'http://www.gravatar.com/avatar/' . md5(strtolower(trim(CWebbit::Instance()->user['email']))) . '.jpg?r=pg&amp;d=wavatar&amp;' . ($size ? "s=$size" : null);
}
...
function create_url($urlOrController=null, $method=null, $arguments=null) {
  return CWebbit::Instance()->CreateUrl($urlOrController, $method, $arguments);
}
...
function theme_url($url) {
  return create_url(CWebbit::Instance()->themeUrl . "/{$url}");
}
...
function theme_parent_url($url) {
  return create_url(CWebbit::Instance()->themeParentUrl . "/{$url}");
}
...
function render_views($region='default') {
  return CWebbit::Instance()->views->Render($region);
}
